...
note-line   = "> note: " free text ;   (* blockquote — renders as an offset aside *)
```
...
v1.1→v1.2: notes are blockquotes — `note:` became `> note: ` (migration:
`sed -i 's/^note: /> note: /' log/*.md`). Conformance is enforced by `scripts/lint-log.ts`
